...
static uint8 registeredSW3KeysTaskID = NO_TASK_ID;
...
uint8 RegisterSW3ForKeys( uint8 task_id )
{
  // Allow only the first task
  if ( registeredSW3KeysTaskID == NO_TASK_ID )
  {
    registeredSW3KeysTaskID = task_id;
    return ( true );
  }
  else
    return ( false );
