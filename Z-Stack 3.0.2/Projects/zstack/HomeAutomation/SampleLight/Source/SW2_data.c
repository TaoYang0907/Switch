...
//global attributes
const uint16 SW2_clusterRevision_all = 0x0001; //currently all cluster implementations are according to ZCL6, which has revision #1. In the future it is possible that different clusters will have different revisions, so they will have to use separate attribute variables.

// Basic Cluster
const uint8 SW2_HWRevision = SAMPLELIGHT_HWVERSION;
const uint8 SW2_ZCLVersion = SAMPLELIGHT_ZCLVERSION;
const uint8 SW2_ManufacturerName[] = { 16, 'T','e','x','a','s','I','n','s','t','r','u','m','e','n','t','s' };
const uint8 SW2_ModelId[] = { 16, 'T','I','0','0','0','1',' ',' ',' ',' ',' ',' ',' ',' ',' ',' ' };
const uint8 SW2_DateCode[] = { 16, '2','0','0','6','0','8','3','1',' ',' ',' ',' ',' ',' ',' ',' ' };
const uint8 SW2_PowerSource = POWER_SOURCE_MAINS_1_PHASE;

uint8 SW2_LocationDescription[17];
uint8 SW2_PhysicalEnvironment;
uint8 SW2_DeviceEnable;

// Identify Cluster
uint16 SW2_IdentifyTime;

// Groups Cluster
uint8 SW2_GroupsNameSupport = 0;
...
uint8  SW2_LevelCurrentLevel;
uint16 SW2_LevelRemainingTime;
uint16 SW2_LevelOnOffTransitionTime;
uint8  SW2_LevelOnLevel;
uint16 SW2_LevelOnTransitionTime;
uint16 SW2_LevelOffTransitionTime;
uint8  SW2_LevelDefaultMoveRate;
#endif

#if ZCL_DISCOVER
CONST zclCommandRec_t SW2_Cmds[] =
{
  {
...
CONST uint8 zclCmdsArraySizeSW2 = ( sizeof(SW2_Cmds) / sizeof(SW2_Cmds[0]) );
#endif // ZCL_DISCOVER
...
CONST zclAttrRec_t SW2_Attrs[] =
{
  ...
  },
  {
    ...
    {  // Attribute record
      ...
      (void *)&SW2_HWRevision  // Pointer to attribute variable
    }
  },
  {
    ZCL_CLUSTER_ID_GEN_BASIC,
  ...
  },
  {
    ...
    { // Attribute record
      ...
      ZCL_DATATYPE_UINT16,
      ACCESS_CONTROL_READ | ACCESS_CONTROL_WRITE,
      (void *)&SW2_LevelOnOffTransitionTime
    }
  },
  {
    ZCL_CLUSTER_ID_GEN_LEVEL_CONTROL,
  ...
  },
  {
    ...
    { // Attribute record
      ...
      ZCL_DATATYPE_UINT16,
      ACCESS_CONTROL_READ | ACCESS_CONTROL_WRITE,
      (void *)&SW2_LevelOnTransitionTime
    }
  },
  {
    ZCL_CLUSTER_ID_GEN_LEVEL_CONTROL,
    { // Attribute record
      ...
      ZCL_DATATYPE_UINT16,
      ACCESS_CONTROL_READ | ACCESS_CONTROL_WRITE,
      (void *)&SW2_LevelOffTransitionTime
    }
  },
  {
    ZCL_CLUSTER_ID_GEN_LEVEL_CONTROL,
...
uint8 CONST SW2_NumAttributes = ( sizeof(SW2_Attrs) / sizeof(SW2_Attrs[0]) );
...
const cId_t SW2_OutClusterList[] =
{
//  ZCL_CLUSTER_ID_GEN_IDENTIFY
  ZCL_CLUSTER_ID_GEN_ON_OFF
};

#define SW2_MAX_INCLUSTERS   (sizeof(SW2_InClusterList) / sizeof(SW2_InClusterList[0]))
#define SW2_MAX_OUTCLUSTERS   (sizeof(SW2_OutClusterList) / sizeof(SW2_OutClusterList[0]))
 
SimpleDescriptionFormat_t SW2_SimpleDesc =
{
  SW2_ENDPOINT,                  //  int Endpoint;
  ...
  SW2_MAX_INCLUSTERS,         //  byte  AppNumInClusters;
  (cId_t *)SW2_InClusterList, //  byte *pAppInClusterList;
  SW2_MAX_OUTCLUSTERS,        //  byte  AppNumInClusters;
  (cId_t *)SW2_OutClusterList  //  byte *pAppInClusterList;
};
...
/*********************************************************************
 * @fn      SW2_ResetAttributesToDefaultValues
 *
 ...
 * @return  none
 */
void SW2_ResetAttributesToDefaultValues( void )
{
  int i;
  
  SW2_LocationDescription[0] = 16;
  for (i = 1; i <= 16; i++)
  {
    SW2_LocationDescription[i] = ' ';
  }
  
  SW2_PhysicalEnvironment = DEFAULT_PHYSICAL_ENVIRONMENT;
  SW2_DeviceEnable = DEFAULT_DEVICE_ENABLE_STATE;
  
#ifdef ZCL_IDENTIFY
  SW2_IdentifyTime = DEFAULT_IDENTIFY_TIME;
#endif
#ifdef ZCL_LEVEL_CTRL
  SW2_LevelCurrentLevel = DEFAULT_LEVEL;
  SW2_LevelRemainingTime = 0;
  SW2_LevelOnOffTransitionTime = DEFAULT_ON_OFF_TRANSITION_TIME;
  SW2_LevelOnLevel = DEFAULT_ON_LEVEL;
  SW2_LevelOnTransitionTime = DEFAULT_ON_TRANSITION_TIME;
  SW2_LevelOffTransitionTime = DEFAULT_OFF_TRANSITION_TIME;
  SW2_LevelDefaultMoveRate = DEFAULT_MOVE_RATE;
#endif
  SW2_OnOff = DEFAULT_ON_OFF_STATE;
